...
#
rm(list=ls())
CE_data <- read.csv("_dataW04/NUTS2_data.csv")
...
MEM.sf <- M1.df[,c(1:9,28)] %>% 
  dplyr::inner_join(df60, ., by = c("NUTS_ID" = "IDs"))  
#
...
#
# 
# We have not performed the "MC-type" Moran eigenvector selection,
