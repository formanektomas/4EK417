...
This tutorial covers the *very basics* of data handling based on the `tidyr` package:  
...
* In **long format** (below), the same data are shown, yet each `Year` and `Country` combination is on a separate row of the dataset. 
    ...
    + In the long format,  columns `Year`, `State` and `GDP` are used in the long format to display the same data.  
    
    + Note that `State` and `GDP` *variables* (i.e. columns in the long-format dataset) need to be created to get the long format (such *variables* do not exist in the wide format dataset)

### GDP - long format (same data)
...
```{r,echo=T}
GDPwide # repeated from above for clarity
```
...
* `key = "State"` is the name of the new key column - a new *variable* (can be any character string you supply). The **keys are generated based on column names of the columns being transformed** (`2:4`)  
    + *variable*-names in the wide format dataframe (`GER`,`FRA`,`USA`) are used as "entries" (keys) in the new column (hence variable name `State`).  

* `value = "GDP"` name of the new value column (can be any character string you supply)  

* `2:4` numeric indexes (or names) of columns to collapse. 

* Please note that column `Year` (1st column) is left out from the syntax (Year is the grey column as in the illustration).  
...
* Please note that `GDPlong` format resembles panel data analysis - both `id` and `time` identification is provided in each row. 

* However, with long-format, we only have one value item in each row. The remaining columns (items in each row) provide identification to our observation. 

* Say, we want to store `GDP` and `Unemployment` in a long format. We would need four columns. For example: `Year`, `State`, `Unit` ("GDP","Unemployment"),`Value` (observed data). 
...
+ `R09_Eurostat_in_depth_data_handling.R` (in your R working directory)  
