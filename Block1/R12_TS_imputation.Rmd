...
As we combine MI and *second-order polynomial time trend*, we may do MI as follows:  

* All available series are used for MI (ID variables (TS and CS) need to be listed separately)  
* `polytime = 2` by default, estimated time-dynamics coefficients apply to all CS units (i.e. are NOT CS-specific -- this will be relaxed next)   
...
```{r}
set.seed(100)
a.out <- amelia(freetrade, m=5, ts = "year", cs = "country", polytime = 2, p2s = 2) # EM: expectation maximization (ML-base method)
summary(a.out)
...
* leads and lags are only used for the `tariff` series (user input, can be amended)
