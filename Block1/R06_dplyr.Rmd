...
* If we want to create a new object using the pipe operater, we do it the usual way: we use `<-` operator and assign a name to the new object:
...
* Retrieve GDP data for Austria: filter for Austria (`"AT"`) AND Years 2015 or later,
...
**Alternative ordering od data.frames to join:** Start with `Unem` and *append* `NUTS2.DF` dataset (`left_join()` works within the pipe operator sequence)
...
* Joining by columns of different names:  
    + Say, `Year` variable is in both DFs, but `NUTS2` IDs are in `geo` column of the second DF:  
    + `left_join(DF1,DF2, by = c("Year", "NUTS2"="geo"))`  
    + With pipes, `DF1` is implicit, comes from the previous "line" of the pipe
    

* Sometimes, `merge()` from the `base` packge may be a reasonable alternative to `join` commands from `dplyr`.  
...
* Filter for year 2015 & NUTS2 regions in Germany
* For the NUTS2 regions above, append all corresponding data from the `Unem` dataset
