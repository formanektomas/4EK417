...
install.packages("eurostat") # instalation from CRAN
```
...
```{r}
Un.DF %>% 
  filter(age == "Y15-74", time >= 2010, sex == "T", isced11 == "TOTAL") %>% 
  filter(geo %in% c("AT","CZ","DE","HU","PL","SK")) %>% 
...
We continue with the previously used Unemployment dataset as in **Example 1** (i.e. not with data from Quick exercise).  
...
Un.2017 <- Un.DF %>% 
  filter(age == "Y15-74", time == 2017, sex == "T", isced11 == "TOTAL") %>% 
...
```{r}
Un.DF %>% 
  filter(age == "Y15-74", time >= 2010, sex == "T", isced11 == "TOTAL") %>% # filter variables
...
Unempl.zoo <- zoo(Unempl[,-c(1:5)], order.by = Unempl$time) 
class(Unempl.zoo)
head(Unempl.zoo)
```
...
Un.2Q <- Un.Q %>% 
  filter(age == "Y15-74", time >= 2010, sex == "T", citizen == "FOR") %>% # filter variables
  filter(geo %in% c("AT","CZ","DE")) %>% # subset of countries
  pivot_wider(names_from = "geo", values_from = "values") # convert to wide format
head(as.data.frame(Un.2Q)) # wide format, not a TS object
# Note the "order.by = as.yearqtr" syntax
# .. I drop redundant columns
Un.Q.zoo <- zoo(Un.2Q[,-c(1:5)], order.by = as.yearqtr(Un.2Q$time)) 
head(Un.Q.zoo)
