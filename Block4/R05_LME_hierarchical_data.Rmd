...
* **Note:** This is how we can compute a new variable `CSES`, the centered individual `SES` using `{dplyr}` package. `CSES` is the relative standing on the `SES` measure for a student within school. We need this so that intercept is the mean achievement for the school.
...
* Note that `Sector` has only two levels (hence, random effects should not be implemented)   
