...
& & \\
\log(\mu_i) &= \beta_0 + \beta_1x_i  & ({\rm counts})&  \\
& & \\
\text{For different exposure times, we can use rates:} & & \\
& & \\
\log\bigg(\frac{\mu_i}{{\rm time}_i}\bigg) &= \beta_0 + \beta_1x_i & ({\rm rates})&  \\
& & \\
 &\Rightarrow  \\
 & & \\
\log(\mu_i) - \log({\rm time}_i) &= \beta_0 + \beta_1x_i  \\
& & \\
 &\Rightarrow  \\
 & & \\
\log(\mu_i) &= \beta_0 + \beta_1x_i + 1\times \log({\rm time}_i) & ({\rm still\ rates})& \\
& & \\
\text{where}~[1\times \log({\rm time}_i)]~\text{is the offset term}. & & \\
& & \\
& & \\
& & \\
\text{ Important note:}& & \\
& & \\
\log(\mu_i) &= \beta_0 + \beta_1x_i + 1\times \log({\rm time}_i) & & \\
& & \\
&\ne  \\
& & \\
\log(\mu_i) &= \beta_0 + \beta_1x_i + \beta_2\times \log({\rm time}_i)\quad {\rm when}\ \beta_2 \ne 1  & ({\rm counts\ again})&
\end{align}
$$
