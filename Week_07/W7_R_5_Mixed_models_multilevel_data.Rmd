...
* 1) we sample individuals  
* 2) individuals aggregated in districts  
* 3) aggregated to several regions  
...
```{r}
MathAchieve <- readRDS("_dataW07/MathAchieve.Rds")
str(MathAchieve)
```
...
$$ \textit{MathAch}_j = \beta_0 + \beta_1 \textit{CSES}_j + \varepsilon_j$$ 

We estimate $\textit{MathAch}$ as a function of relative socioeconomic status. This model ignores that the students attend different schools. Please note that index $j$-th index corresponds to individuals (students).
...
$$y_j = \beta_{0i} + \beta_1 x_{ij} + \varepsilon_j$$
where the index $i$ indicates the school of the student.

With $\beta_{0i}$, we specify a separate intercept for each school. Indeed, what we include is a categorical variable with as many categories as schools. The next model considers `School` as a fixed effect.
...
You can see that the amount of $\beta_{0i}$ parameters estimated is overwheliming. 

We could generalize (i.e. complicate) the model by allowing for different `CSES` slope for each school (note the index $i$ for the slope $\beta_{1i}$):


$$y_{ij}=\beta_{0i} + \beta_{1i}x_{x_{ij}} + \varepsilon_{ij}$$
...
In most cases, we are not interested in these schools in particular, but in the population of schools in order to compare schools with different characteristics.
...
##### Next Figure illustrates `MathAchieve` variability by schools and within schools (we took a subsample)
...
* Level 1: $y_{ij} = \mu_i + \varepsilon_{ij}$
    + where index $j$ corresponds to individuals and $i$ to schools.
    + For the simplest model, individual `MathAchieve` is given as School average + individual error.
    + If we consider Schools as a random effect, then $\mu_i$ (the average of each school) would be given by:
  
* Level 2: $\mu_i = \beta_0 + u_i$
    + where $\mu_i$ is average of each school,
    + $\beta_0$ is total average of all schools (all students in all schools), 
    + $u_i$ is deviation of $i$-th school from total average. 
    
    
Now, we can write the previous model as: 

$$ y_{ij} = \beta_0 + u_i + \varepsilon_{ij}, \quad i=1,\dots,m \quad j=1,\dots,n_m$$

At both levels, we assume 

* $u_i\sim N(0,\sigma^2_{u_i})$ and 
* $\varepsilon_{ij} \sim N(0,\sigma^2)$ 
* (both independent).

The mean of $y$ for the $i$-th group is given by $\beta_0 + u_i$ , then $u_i$ is the deviation of the mean of $i$-th group from the total average $\beta_0$.

Individual level residuals $\varepsilon_{ij}$ are the difference between the value of the response variable of the $j$-th individual and the average of the group (School) they belong to. 


#### We fit the model with `lme`
* Please note that the `lm` counterpart to this model is
* `lm(MathAch~1,data=MathAchieve)`
    + i.e. model with Intercept as a fixed effect, no random effects (CSES ignored)
...
##### Hence, $\mu_i = 12.637 + \hat{u}_i$

##### Residuals $\hat{u}_i$ may be extracted as:
```{r}
fixef(ml1)-(predict(ml1, newdat=list(School=c("91","3","31","52","74"))))
```


##### Plot of $\hat{\mu}_i$ and confidence intervals (using `lmer()` package)
...
```{r}
summary(lm(MathAch~1, data=MathAchieve))$sigma
summary(lm(MathAch~1, data=MathAchieve))$sigma^2
var(MathAchieve$MathAch)
```

Considering a random effects model, `ml1 <- lme(MathAch~1,random=~1|School, ...)`, we have $\sigma^2_u$ and $\sigma^2$. 

The total variability of the data is:
$var(y_{ij}) = \sigma^2 + \sigma^2_u$ i.e. the variability within the schools plus the variability among the schools. 

```{r}
VarCorr(ml1)
```

Note the difference between `var(MathAch)` and $\hat{\sigma}^2 + \hat{\sigma}^2_u$ is related to the ML estimation method, see e.g. discussion [here](https://stats.stackexchange.com/questions/275620/why-is-the-estimated-variance-of-a-random-effect-not-equal-to-the-variance-of-it)

***

#### The intra-class correlation coefficient ($ICC$) is
$$\textit{ICC}=\frac{\sigma^2_u}{\sigma^2_u+\sigma^2} = \frac{2,93^2}{2,93^2+6,26^2} \doteq 0,18$$

Hence, 18% of the total variability is due to the difference among (the averages of the) schools.
...
$H_1: y_{ij} = \beta_0 + u_i + \varepsilon_{ij}.$
...
### CSES as fixed effect

* Level 1:  $y_{ij} = \mu_i + \beta_1 x_{ij} + \varepsilon_{ij}$
    + individual math score as a function of school average and individual's CSES.
    + slope $\beta_1$ is assumed identical for all schools
* Level 2:  $\mu_i = \beta_0 + u_i$


Putting both equations together, we have:
$$y_{ij}=  \underbrace{\beta_0 + \beta_1 x_{ij}}_{\textit{fixed effects}} + \underbrace{u_i }_{\textit{random effect}}+\varepsilon_{ij}$$
...
$\beta_0$ is the average math achievement for those students with average socioeconomic status. The average straight line is given by:

$$\textit{MathAch} = 12.62 + 2.19 \cdot \textit{CSES}$$
...
In order to constrast if the slope is significatively not equal to zero, we need to perform a Likelihood Ratio Test for the fixed effects by means of maximum likelihood (**ML not REML**):
...
We generalize the previous model by allowing for a different random slope for each group (school): 

* Level 1: $y_{ij} = \mu_i + \beta_{1i} x_{ij} + \varepsilon_{ij}$
* Level 2: $\mu_i = \beta_0 + u_i$
* $\beta_{1i} = \beta_1 + v_i$
...
Putting the equations together, we have:
 ...
 $$y_{ij}=  \underbrace{\beta_0 + \beta_1 x_{ij}}_{\textit{fixed effects}} + \underbrace{u_i + v_i x_{ij} }_{\textit{random effects}} + \varepsilon_{ij}$$
...
where $\sigma_{uv}$ is the covariance between the intercepts of the groups and the slopes. A positive value of the covariance implies that groups with high $u_i$ effect tend to have high values of the $v_i$ effect, or equivalently, school with high intercepts also have higher slopes.
...
In order to constrast if the random effect in slope is justified (against fixed slopes):
...
$\sigma_{uv}$ (the covariance between the intercepts of the groups and the slopes) is estimated. 

In order to simplify our model and its interpretation, we may want to test $H_0: \sigma_{uv} = 0$ against  $H_1: \sigma_{uv} \neq 0$:
...
* Level 1: $y_{ij} = \mu_i + \beta_{1i}x_{ij} + \varepsilon_{ij}$
* Level 2: $\mu_i = \beta_0 + \beta_2 s_i+ u_i$
* $\beta_{1i} = \beta_1 + \beta_3 s_i + v_i$

Again, this can be combined into:

 $$y_{ij}=  \underbrace{\beta_0 + \beta_1 x_{ij} +\beta_2 s_i + \beta_3 (x_{ij} \times s_i)}_{\textit{fixed effects}} + \underbrace{u_i + v_i x_{ij}}_{\textit{random effects}}+ \varepsilon_{ij}$$
...
This example is based on lecture by [Dae-Jin Lee](http://idaejin.github.io/bcam-courses/neiker-2016/material/mixed-models/) and slightly modified. [cc](https://creativecommons.org/licenses/by-sa/3.0/)
