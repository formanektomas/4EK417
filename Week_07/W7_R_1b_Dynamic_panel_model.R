...
ggplot(EmplUK, aes(x=as.factor(year), y=log(emp), fill=as.factor(year))) + 
  ...
  theme_minimal() 
#
# Estimation of the dynamic model using GMM - Arellano and Bond esstimation
...
#
# Wald tests performed separately
plm:::wald(D.P.2, "coef", vcovHC) # H_0 of \beta_j = 0 \forall j
plm:::wald(D.P.2, "time", vcovHC) # H_0 of not signif. time-dummies
plm:::wald(D.P.2, "all", vcovHC)  # H_0 combines two previous hypotheses.
#
#
#
#
