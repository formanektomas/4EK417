...
ggplot(plotDF.SP1, aes(x = age, y = wage) ) +
  ...
  theme_bw()
#
# We can verify that the two fitted curves are actually identical:
#
...
#
SPfit2 <- lm(wage~bs(age, knots=c(25,40,60)), data = Wage) #manual knots
predSP2 <- predict(SPfit2, interval = "confidence")
plotDF.SP2 <- cbind(Wage, predSP2)
#
# 3 knots set automatically
#
SPfit3 <- lm(wage~bs(age, df=6), data = Wage) #automatic knots
attr(terms(SPfit3), "predvars") # retrieve knot information
...
+ Use 4 knots (either provide the knots explicitely or using`df` argument)
...
For basis functions, we only use knots up to $K\!-\!2$.
...
$$
y_i = \beta_0 \, N_1 (x_i)  + \beta_1 \, N_2 (x_i) + \beta_2 \, N_3 (x_i) + u_i  
$$
...
attr(terms(fitNS2), "predvars") # 1 internal knot, 2 boundary knots with nat. restrictions
...
+ Using default `ns()` settings, the model would be estimated as follows (`no intercept` is the default, hence the `df` argument adjustment):

```{r}
fitNS3 <- lm(wage~ns(age,df=4), data = Wage) # knots = df-intercept-1
...
+ Cross-validation can be used to find $\lambda$ with the lowest cross-validated RSS (LOOC is computationally very efficient here).
...
```{r}
SSfit <- smooth.spline(age,wage,cv=F) # smoothing splines
# as age values are repeated (x-axis), do NOT use LOOC, i.e. "cv=T".
