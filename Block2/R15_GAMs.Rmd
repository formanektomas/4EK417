...
+ Instead, we fit all $p$ functions "simultaneously", using a scatterplot smoother (e.g. smoothing spline, natural spline, polynomial function, etc.) , using a **backfitting algorithm**:
...
+ Each of the $f_j$ functions is a cubic spline (with knots at each unique value of $x_j$ and $\textit{df}_{\lambda}$).  

+ Without further restrictions, solution is non-unique -- constant $\alpha$ is non-identifiable as we can add/subtract any constant to each of the $f_j$ functions.    

+ This is solved in Step 1 (i.e. by setting $\hat{\alpha} = \text{mean}(y_i)$ and $\hat{f}_j \equiv 0 $). Note that this setting does not change troughout the backfitting estimation.  

+ Step 2 (a) applies a cubic smoothing spline $\mathcal{S}_j$ to the $X_j$ regressor while fixing all other $\hat{f}_k$ at their current estimates when computing the $\left\lbrace y_i - \hat{\alpha} - \sum_{k \neq j} \hat{f}_k (x_{ik}) \right\rbrace_1^n$ term.  
...
where $f_1$ and $f_2$ are (cubic) smoothing splines and $f_3$ is a step function. 
...
# 
# significance of different constituent elements of the model
#
sum.gam$parametric.anova
#
# H_0 of linear relationship against H_1 of non-linear relationship
#
sum.gam$anova
```


+ Given the results of diagnostic tests, let's drop the smoothing spline for $\texttt{year}$ and simply use a linear function instead: 
...
ggplot(Wage, aes(x = Gam.pred$fit, y = Wage$wage) ) +
  geom_point(color="darkgreen")+
  ylim(0,320) +
  xlim(0,320) +
  coord_fixed() + # 1:1 x/y aspect ratio
  geom_segment(  aes(x=0, y=0, xend=320, yend=320), size = 0.7, color='red', linetype = "dotted")+
...
## GAM based on a combination of splines and local regression

+ We can easily combine different types of smoothers in a GAM:

```{r, warning=F}
gam4 <- gam(wage~s(year,df=2)+lo(age,span = 0.2)+education, data=Wage)
par(mfrow=c(1,3))
plot(gam4, se=T, col="blue") # syntax simplified since gam2 is a gam object
par(mfrow=c(1,1))
summary(gam4)
```
...
+ Interpret the above diagnostic tests in `summary(gam4)` and make ammendments the the `gam4` specification as necessary.



--- 

#### GAMs can encompass interaction elements:

+ Interaction between $\texttt{age}$ and $\texttt{year}$ can be a{ccounte for as follows (`{gam}` package based example, ISLR textbook)  


```{r, warning=F}
gam5 <- gam(wage~lo(age,year,span = 0.4)+education, data=Wage)
summary(gam5)
