...
```{r}
x <-  model.matrix(Salary~., Hitters)[ , -1] # 1st col. is for the dependent variable Salary
y <- Hitters$Salary
```
...
* At the top of the plot, you can see how many non-zero variables coefficients are in the model. Here, there's all 20 variables in the model (19 variables plus the intercept) and no coefficient is zero (as we are doing ridge regression, regressors are not turned off).
