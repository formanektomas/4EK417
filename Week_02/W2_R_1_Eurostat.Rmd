...
R tools to access open data from [Eurostat](http://ec.europa.eu/eurostat). 
...
* e.g. for Unemployment at NUTS 2 level.


```{r}
# kable() generates tabular output in Rmd files
...
We can use `{tidyverse}` functions to filter and plot data. 

**Example 1: **

* `Y15-74` i.e.  *From 15 to 74 years*  
...
Un.DF %>% 
  ...
  subset(geo == "AT" | geo == "CZ" | geo == "DE" | geo == "HU" | geo == "PL" | geo == "SK") %>% 
...
Un.2016 <- Un.DF %>% 
  ...
  filter(NUTS0=="AT" | NUTS0=="CZ" | NUTS0=="DE" | NUTS0=="HU" | NUTS0=="PL" | NUTS0=="SK")
