...
#
library(boot)
?cv.glm # please make sure to read "Arguments" description
#
# Since the response is a binary variable, an appropriate cost function is
Cost <- function(r, pi = 0) mean(abs(r-pi) > 0.5)
# .. cost function simply repeated - it is the same for cvFit() and cv.glm() 
#
...
#
rm(list=ls())
Titanic <- read.csv("_dataW08/titanic_cleaned.csv")
#
head(Titanic)
summary(Titanic)
attach(Titanic)
...
## to compare cross-validated error rates for 
## logistic regression models LR1 to LR3
#
#
#
#
