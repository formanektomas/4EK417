...
#
# First, we need to create the alternative predictions:
pred2 <- rep(0, length(data$log.fit))
for(i in 1:length(data$log.fit)) {
  if(data$log.fit[i] > .3) {
    pred2[i] <- 1
  } else {
    pred2[i] <- 0
...
pred3 <- rep(0, length(data$log.fit))
for(i in 1:length(data$log.fit)) {
  if(data$log.fit[i] > best.acc.cutoff) {
    pred3[i] <- 1
  } else {
    pred3[i] <- 0
  }
