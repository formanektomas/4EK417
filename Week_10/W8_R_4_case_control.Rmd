...
rm(list=ls())
heart <- read.csv("_dataW08/SouthAfrica.csv")
head(heart)
str(heart)
...
and for "the same" average male without family history of MI


```{r}
attach(heart) 
# We append two rows at the end of "heart" dataframe:
# first is for average male with MI family history "Present"
# second is for average male with MI family history "Absent"
...
Biased predictions, based on **sample** "success" prevalence (beta 0 is biased) -- no case-control correction:
...
heart3$famhist <- 0  # with dummy variables instead of the factor
heart3$famhist[heart2$famhist == "Present"] <- 1
...
* The simples way to proceed with this assignment is to amend `R` starting on line `[154`].  
