...
#
#
# Declare another simple function - no arguments
myFunction2 <- function() {
  myNoise2 <- rnorm(1000, mean=10, sd=0.5) 
  print("Mean")
  cat("Sample mean is", mean(myNoise2), "\n") # "\n" is for inserting a new line
  print("Variance")
  cat("Sample variance is", var(myNoise2))
} 
# Run a function
myFunction2()
#
head(myNoise2) # Shows first six values of myNoise2 .... does it?
#
# Objects created within a function are saved within that function's 
# environment, NOT to the Global Environment,
# unless the <<- operator is used. 
# generally, using the "super-assignment" operator is not recommended.
#
# Please, remember:
myFunction2 # this command does NOT execute the function because of missing "()"
#
#
#
#
...
#
# 
# Example 1: We need to calculate and store 1st differences
# of some variables stored (in columns)  from a matrix,
# for subsequent analysis.
#
m <- matrix(rnorm(25), ncol= 5)
m
dim(m)
dm <- diff(m) # column-wise differences, e.g. dm[1,1] = m[2,1]- m[1,1] 
dm
dim(dm)
# If you assumed the following output for dm, you may get error messages
# and/or wrong results from subsequent calculations, plots, etc.
rbind(c(NA,NA,NA,NA,NA), dm) 
#
# print(object) and str(object) commands are the first-choice debug tools.
#
#
#
# Example 2: We need to compare two vectors (but fail to control for equal lenghts)
#
a <- c(1:10)
b <- c(12:0)
#
